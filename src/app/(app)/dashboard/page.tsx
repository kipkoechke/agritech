...
export default function TeaDashboardPage() {
  ...
  const trendData = useMemo(() => {
    ...
    filteredData.forEach((item) => {
      ...
      zonesMap[item.zone][item.date] =
        (zonesMap[item.zone][item.date] || 0) + item.kilos;
    });
  ...
  const trendZones = useMemo(
    () => [...new Set(filteredData.map((d) => d.zone))],
    [filteredData],
  );
  ...
  const topFactories = useMemo(() => {
    ...
    filteredData.forEach((d) => {
      map[d.factory] = (map[d.factory] || 0) + d.kilos;
    });
  ...
  const zoneDistribution = useMemo(() => {
    const map: Record<string, number> = {};
    filteredData.forEach((d) => {
      map[d.zone] = (map[d.zone] || 0) + d.kilos;
    });
  ...
  const supervisorPerformance = useMemo(() => {
    const map: Record<string, number> = {};
    filteredData.forEach((d) => {
      map[d.supervisor] = (map[d.supervisor] || 0) + d.kilos;
    });
    return Object.entries(map)
  ...
  const formatKilosLower = (kilos: number) =>
        ...
        {(isAdmin || isFarmer) && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            <RankingChart
              ...
              title={
                isFarmer
                  ? "My Factories by Collection"
                  : "Top Factories by Collection"
              }
              icon={MdBusiness}
              valueLabel="KG"
            />
            <RankingChart
              ...
              title={
                isFarmer
                  ? "My Farmers by Production"
                  : "Top Farmers by Production"
              }
              icon={MdPerson}
              valueLabel="KG"
            />
          ...
          <div className="bg-white rounded-lg border border-slate-200 mb-4">
            ...
            <div className="p-3 md:p-4">
              <div className="h-[500px] rounded-lg overflow-hidden border border-gray-200">
                <MapContainer
                  center={[0.2, 35.5] as L.LatLngExpression}
                  zoom={7}
                ...
                >
                  ...
                  {mapMarkers.map((marker) => (
                    <CircleMarker
                      key={marker.id}
                      center={[marker.lat, marker.lng] as L.LatLngExpression}
                      radius={Math.sqrt(marker.totalKilos) / 2}
                      fillColor={getMarkerColor(marker.totalKilos)}
                      color={getMarkerColor(marker.totalKilos)}
                      weight={2}
                      opacity={1}
                      fillOpacity={0.7}
                    >
              ...
              <div className="mt-3 flex flex-wrap gap-4 justify-center">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">High (400+ KG)</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        {isPlucker && (
          <div className="bg-white rounded-lg border border-slate-200 p-8 text-center">
            ...
            <p className="text-gray-500">
              Your services and tasks will appear here.
            </p>
          </div>
        )}
      </div>
