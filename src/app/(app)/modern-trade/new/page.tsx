...
import { useCreateMTCustomer } from "@/hooks/useMTCustomer";
import { useSalesPersons } from "@/hooks/useSalesPerson";
import { useZones } from "@/hooks/useZone";
import { useAuth } from "@/hooks/useAuth";
import type { CreateMTCustomerPayload } from "@/types/mtCustomer";
...
export default function NewMTCustomerPage() {
  const router = useRouter();
  const { user } = useAuth();
  const createMutation = useCreateMTCustomer();
  ...
  const onSubmit = (data: MTCustomerFormData) => {
    setApiErrors({});
    const payload: CreateMTCustomerPayload = {
      name: data.name,
    ...
    createMutation.mutate(payload, {
      ...
      onError: (error: unknown) => {
        ...
        if (axiosError.response?.data?.errors) {
          ...
          toast.error("Failed to create MT customer");
        }
      },
    });
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-0.5">
                Create a new modern trade customer account
              </p>
            </div>
          </div>
        </div>
