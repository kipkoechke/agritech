...
import { useSalesPerson, useUpdateSalesPerson } from "@/hooks/useSalesPerson";
...
interface EditSalesRepresentativePageProps {
  params: Promise<{ id: string }>;
}

export default function EditSalesRepresentativePage({
  params,
}: EditSalesRepresentativePageProps) {
  const { id } = use(params);
  const router = useRouter();

  const { data: person, isLoading } = useSalesPerson(id);
  ...
  useEffect(() => {
    if (person) {
      reset({
        name: person.name,
        email: person.email,
        zone_id: person.zone?.id || "",
        phone: person.phone || "",
        status: person.status || "active",
        contact_phone: person.contacts?.phone || "",
        contact_email: person.contacts?.email || "",
