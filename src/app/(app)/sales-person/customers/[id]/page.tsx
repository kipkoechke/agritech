...
export default function SalesPersonCustomerDetailPage({
  params,
}: CustomerDetailPageProps) {
  const { id } = use(params);
  const [ordersPage, setOrdersPage] = useState(1);

  const { data: customer, isLoading, error } = useSalesPersonPortalCustomer(id);
  const { data: ordersData, isLoading: ordersLoading } =
    useSalesPersonCustomerOrders(id, { page: ordersPage, per_page: 10 });
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            KES {parseFloat(order.amount).toLocaleString()}
                          </p>
                        </div>
