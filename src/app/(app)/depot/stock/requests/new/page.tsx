...
import { useCreateStockRequest } from "@/hooks/useStockRequest";
...
import type { CreateStockRequestPayload, ProductRequest, StockRequestBatch } from "@/types/stockRequest";
...
export default function NewStockRequestPage() {
  ...
  // Get stock levels for the selected source zone (from_zone_id)
  ...
  const handleProductChange = (index: number, field: keyof ProductRequest, value: any) => {
    ...
    // If changing product_id, check for duplicates
    if (field === "product_id" && value) {
      const isDuplicate = newProducts.some((product, i) =>
        i !== index && product.product_id === value
      );
    ...
    // Reset batches if product changes
    if (field === "product_id") {
  ...
  const removeProduct = (index: number) => {
    ...
    // Remove batch selection state for this product
  ...
  const validateForm = () => {
    ...
    if (!formData.from_zone_id) newErrors.from_zone_id = "Source zone is required - select which zone to request stock from";
    if (!formData.to_zone_id) newErrors.to_zone_id = "Your zone should be automatically set";
    if (formData.from_zone_id === formData.to_zone_id) {
      newErrors.from_zone_id = "Cannot request stock from your own zone - please select a different source zone";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Clean up products data - remove empty batches arrays
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">You don't have permission to create stock requests.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Request stock transfer from another depot"
      />
              ...
              <div className="space-y-4">
                        ...
                        <>
                          ...
                          <button
                            ...
                            title={allSelected
                              ...
                              : "Add another product to this request"}
                          >
                ...
                {formData.products?.map((product, productIndex) => (
                  <div key={productIndex} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          {(() => {
                            const availableProducts = productsData?.data?.filter(prod => {
                              // Don't show products that are already selected in other rows
                              const selectedProductIds = formData.products
                                ?.map((p, index) => index !== productIndex ? p.product_id : null)
                                .filter(Boolean) || [];
                    ...
                    {/* Batch Selection for this product */}
                    {product.product_id && (
                      <div className="mt-4 space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() => setShowBatchSelection(prev => ({
                              ...prev,
                ...
                {errors.products && (
                  <p className="text-sm text-red-700 font-medium">{errors.products}</p>
                )}
              </div>
