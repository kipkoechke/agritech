...
import { MdPeople, MdPerson, MdBadge } from "react-icons/md";
import Link from "next/link";
import { useDepotSalesPersons } from "@/hooks/useDepotPortal";
...
export default function DepotSalesPersonsPage() {
  ...
  const { data, isLoading, error } = useDepotSalesPersons({
    page,
    per_page: perPage,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-600 mt-0.5">
                View sales representatives in your depot
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
              ...
              {data.data.map((rep) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <Link
                      href={`/depot/sales-persons/${rep.id}`}
                      className="text-sm font-bold text-primary hover:underline"
                    >
                      {rep.employee_number}
                    </Link>
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusStyle(rep.status)}`}
                    >
                      {rep.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500">{rep.zone?.name || "-"}</span>
                    <span className="text-gray-500">{rep.customers_count} customers</span>
                  </div>
                </div>
              ))}
            </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.data.map((rep) => (
                    <tr key={rep.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getStatusStyle(rep.status)}`}
                        >
                          {rep.status}
                        </span>
                      </td>
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-gray-600">
              {debouncedSearch
                ...
                : "No sales representatives in your depot yet."}
            </p>
          </div>
        )}
      </div>
