...
interface PerformanceReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerate: (params: PerformanceReportParams) => void;
...
const REPORT_OPTIONS = [
  {
    ...
    description: "All orders grouped by sales representative",
    icon: MdShoppingCart,
    color: "text-blue-600",
    bgColor: "bg-blue-50",
  },
  {
    ...
    label: "Unpaid Balances Report",
    description: "Dispatched orders with outstanding balances",
  ...
  },
  {
    ...
    label: "Paid Orders Report",
    description: "Dispatched orders that have been fully paid",
    icon: MdPaid,
...
export default function PerformanceReportModal({
  ...
  const [selectedReport, setSelectedReport] = useState("total_orders");
  ...
  }, [selectedReport, dateFrom, dateTo, onGenerate]);

  const handleClose = useCallback(() => {
    if (!isGenerating) {
      setSelectedReport("total_orders");
      setDateFrom("");
      setDateTo("");
      onClose();
    }
